...
it stays in `secrets.yaml` under the same name with `_old` appended, so no key lands in the device yaml. A key that
comes from a substitution or a package is refused. A failure before the upload puts the previous key back; after the
