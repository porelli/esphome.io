...
`!secret` on it points to, and the key being replaced stays in the configuration as `old_key:`. A key that comes
from a substitution or a package is refused. A failure before the upload puts the previous key back; after the
