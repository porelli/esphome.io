...
Both paths below can be done in one install instead: add `encryption:` together with `allow_plaintext_upload: true`,
...
- The step 1 upload is plaintext, the running firmware does not offer encryption yet, and validation warns about the
  ...
  2027.3.0, so do step 1 before then; after that use `allow_plaintext_upload: true` as described above.
