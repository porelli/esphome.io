...
old one. Put the new key in `key:` and move the previous one to `old_key:`, then install. The CLI presents the new key
first, and when the device rejects it reconnects once with `old_key:`. The firmware it sends carries the new key only;
`old_key:` never reaches the device.
...
After a successful install the device runs the new key and the CLI says so; remove `old_key:` then, an `old_key:`
equal to `key:` is rejected at validation. The Device Builder writes `old_key:` itself when a key is rotated there,
for example when Home Assistant hands over a new API key, so the next install still reaches the device. With the recommended shape the API key changes too, so Home Assistant needs
...
[`--prompt-ota-key`](/guides/cli/#upload-command) instead of editing the configuration.
