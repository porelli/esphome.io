...
- **encryption** (*Optional*): Encrypt the OTA update with the same Noise protocol the [native API](/components/api) uses. See [Encryption](#encryption) below. Cannot be combined with **password**, except with **allow_plaintext_upload** for the migration install.
...
keep any `password:`, and install. The CLI sends that one install in plaintext because the device cannot encrypt yet,
